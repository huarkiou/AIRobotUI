...
- 同 exe 双模式：无参数启动 GUI，带参数启动 CLI
- CLI 通过 HTTP (`127.0.0.1:<port>`) 与运行中的 GUI 实例通信
- 端口发现：GUI 启动时将端口写入 `cli_port.txt`，CLI 读取后连接
- 所有 CLI 操作 marshaling 到 tkinter 主线程执行，无竞态
- CLI 命令：`list` `status <name>` `start <name>` `stop <name>` `restart <name>` `webui <name>` `reload`
- CLI 用 `argparse` 子命令，`--help` 输出用法
- `--windowed` 打包的 exe 在 CLI 模式下通过 `AttachConsole(-1)` 连接父控制台输出
...
- toggle 窗口：`_visible` 为 True 时直接 `hide()`，不要用 `after()` 延迟隐藏——会造 flash 后消失的视觉 bug
